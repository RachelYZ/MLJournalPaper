...
texreg(list(reg[[1]], reg[[2]], reg[[3]], reg[[4]], reg[[5]], reg[[6]], reg[[7]], reg[[14]],reg[[21]],reg[[30]],reg[[60]],reg[[90]]), booktabs = ROH_0)
@
...
for (i in agg){
  ...
  nn.model = neuralnet(ROH_0~., this.train, linear.output = F, hidden=3, stepmax = 1e+06)
...
#ran.nn.pre.scaled <- read.csv("~/e2.ran.nn.pre.csv", header=ROH_0)

ran.nn.pre.us = ran.nn.pred*(scale.list.e2$`scaled:scale`)+
  scale.list.e2$`scaled:center`
ran.nn.pre.us[,1] = ran.test$ROH_0
