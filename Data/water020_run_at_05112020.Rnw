...
colnames(wide) = c('ROB0', 'DOW', paste0('ROB',agg))
...
ran.train = wide[tr_ind, ]
nrow(ran.test)
ran.test = wide[-tr_ind, ]
@
...
#varImpPlot(rtt)
#varImp(r.tree)
write_csv(ran.rf.pred, 'rf05122020.csv')
ran.rf.pred=read_csv('rf05122020.csv')
