...
for (i in rev(agg)){
  ...
  hidden = round((14-match(i, agg)+6)*2/3, 0) #see literature review
  this.train = nn.train.scaled[, c(1:8, colind:ncol(nn.train.scaled))]
  nn.model = neuralnet(ROH0~., this.train, linear.output=T, hidden=hidden)
...
# time.rf = Sys.time() - s1 
# 
# write_csv(rf.pred, 'rfpred_wd_0512.csv')
rf.pred <- read.csv("rf_wd_pred_new_0512.csv")
