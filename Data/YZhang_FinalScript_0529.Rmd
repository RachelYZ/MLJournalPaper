---
title: "Pick-up + Machine Learning: A Proved Efficient Approach to Forecast Hotel Demand "
author: "Rachel Zhang"
date: "05/29/2020"
output:
...
```{r opkknn}
kable(opk, 'latex', caption = 'Parameter selection of K for K-NN model',
        longtable = F, booktabs = T) %>%
  kable_styling(latex_options = c("striped", "hold_position", "scale_down" ,"repeat_header"))
```

For Weighted K-Nearest Neighbors: 
...
For Random Forest, this following chart shows the number of variables selected when building each tree: 

```{r rf2}
set.seed(0)
rf.pre2 = test
rf.pre2[, 3:ncol(test)] = NA
mtry.rf = rep(NA, length(agg))
...
This following chart shows the gamma selected for each model
