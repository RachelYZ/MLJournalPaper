...
This analysis is conducted under `R version 3.6.0` with `set.seed(0)`. The robust test in Section X is conducted under `set.seed(123)`. 
...
for (i in agg){
  colind=which(names(train)==paste0('ROH',i))
  this.train = train[, c(1, 2, colind)]
  svr.model = tune.svm(ROH0~., data=this.train, kernel="radial",
                 gamma=c(0.1,2^(-(0:5))))    
  gamma[[colind-2]] = svr.model$best.parameters[1,1]#  svr.tune = tune(svr, ROH0~., data = this.train, 
 #                 kernel = 'polynomial', ranges = list(degree = 1:5, coef0 = c(0.5,1,2)))
  svr.pre2[, colind] = predict(svr.model$best.model, test)
}
svr.err2 = materr(svr.pre2[,-2]) %>% t() %>%
  as.data.frame()
time.svr = Sys.time() - s1
gamma
# #select among linear, 
# svr.linear = tune.svm(ROH0~., data=this.train, kernel="linear", cost=c(0.001, 0.01, 0.1, 1,5,10))
# svr.linear$best.performance #5.38 - 9.66 - 18.5 - 143 
# 
# svr.poly = tune.svm(ROH0~., data=this.train, kernel="polynomial", 
#                     degree=c(2,3,4,5), coef0=c(0.1,0.5,1,2,3,4))
# svr.poly$best.performance  #5.28 - 9.55 - 17.3 - 149
# 
# svr.radial = tune.svm(ROH0~., data=this.train, kernel="radial",
# gamma=c(0.1,0.5,1,2,3,4))
# svr.radial$best.performance  #19.2 - 22.4 - 150
# 
# svr.sigmoid = tune.svm(ROH0~., data=this.train, kernel="sigmoid",
# gamma=c(0.1,0.5,1,2,3,4), coef0=c(0.1,0.5,1,2,3,4))
# svr.sigmoid$best.performance  #very bad - 23.9 - 158
```
...
ME_ALL = cbind(apk = apk.err[-1,1], mpk=mpk.err[-1,1],
              ...
              rf = rf.err2[-1,1], svr = svr.err2[-1,1]) %>%
  as.data.frame()
...
MAE_ALL = cbind(apk = apk.err[-1,2], mpk=mpk.err[-1,2],
              ...
              rf = rf.err2[-1,2], svr = svr.err2[-1,2]) %>%
  as.data.frame()
rownames(MAE_ALL) = c(paste0('DBA',agg))
#
MAEmelt = melt(as.matrix(MAE_ALL[1:7,]), varnames=c('DBA', 'Model'))
...
SDE_ALL = cbind(apk = apk.err[-1,5], mpk=mpk.err[-1,5],
              ...
              rf = rf.err2[-1,5], svr = svr.err2[-1,5] ) %>%
  as.data.frame()
...
MPE_ALL = cbind(apk = apk.err[-1,3], mpk=mpk.err[-1,3],
              ...
              rf = rf.err2[-1,3], svr = svr.err2[-1,3]) %>%
  as.data.frame()
...
MAPE_ALL = cbind(apk = apk.err[-1,4], mpk=mpk.err[-1,4],
              ...
              rf = rf.err2[-1,4], svr = svr.err2[-1,4]) %>%
  as.data.frame()
